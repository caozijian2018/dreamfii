  ...
  <div class="bigbox">
      ...
      <div class="display_flex head_innerbox  flex_jusify_space">
        <div class="display_flex   flex_align_center">
          ...
          <div class="display_flex flex_align_center classfication_pc">
            ...
            <div class="cate_div" v-if="isTest || show_login_button" @mouseover="orderselect('login')" @click="showLogin()"
            ...
            <div class="cate_div" v-if="show_unlogin_button" @mouseover="orderselect('login')" @click="unLogin()"
                 @mouseleave="selectOrder=''">
              <div class="hover_cate_back cataback" :class="{hover_cate_back_select:selectOrder=='login'}">
              </div>
              <span class="pcs">
        ...
        <div class="head_icon display_flex">
          <i :class="{selectback_ground:showWhich=='search'}" @click.stop="showWichbox('search')"
             class="iconfont navigator icon-fangdajing"></i>
          <div class="option_animate display_flex pc_option_none  flex_align_center"
    ...
    <div class="option_box" :class="{select:showWhich=='option'}">
      <div class="width_90 margin_auto">
        ...
        <video-button v-if="isTest || show_login_button" :button-text="$t('words.login')" @click.native="showLogin()"
                      class="margin_top_1"></video-button>
        <video-button v-if="show_unlogin_button" :button-text="$t('words.unlogin')" @click.native="unLogin()"
                      class="margin_top_1"></video-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      checkTest() {
        if(location.href.toLowerCase().match(/test=1/)){
          if (location.href.toLowerCase().match(/test=1/)[0] === 'test=1') {
            this.isTest = true
          }
        }
      },
